...
def precompute_river_basin():
    ...
    if SMALL_RIVER_BASIN_SHP is not None:
        ...
        MAP_STATS.small_boundary_mask = np.zeros(out_shape, dtype=np.int32)
        MAP_STATS.small_outlet_mask = np.zeros(out_shape, dtype=bool)
    if LARGE_RIVER_BASIN_SHP is not None:
        ...
        MAP_STATS.large_boundary_mask = np.zeros(out_shape, dtype=np.int32)
        MAP_STATS.large_outlet_mask = np.zeros(out_shape, dtype=bool)
