...
def soc_dynamic_model(E_tcell, A, V, month, year, past=False, LAI_avg=None):
    ...
    if past:
        ...
        # ------------------------------------------------------------
        # Always define equilibrium SOC prior safely
        # ------------------------------------------------------------
        if RUN_FROM_EQUIL and len(MAP_STATS.C_fast_equil_list) > month:
            soc_equil_fast = MAP_STATS.C_fast_equil_list[month]
            soc_equil_slow = MAP_STATS.C_slow_equil_list[month]
        else:
            # fallback when you did not run equilibrium
            soc_equil_fast = soc_past_fast
            soc_equil_slow = soc_past_slow
