...
# ▶ NEW: annual mean dam remained capacity by scenario
DAM_REM_CSV = OUTPUT_DIR / 'annual_mean_dam_rem_cap_by_scenario.csv'

years = np.arange(1950, 2025)  # 1950–2023
...
def load_df():
    ...
    # ▶ NEW: DAM remained capacity (annual mean of monthly sums), Past (≤2006) / Present (≥2007)
    df_dam = pd.read_csv(DAM_REM_CSV)
    dam = pd.Series(index=years, dtype=float)
    for y in years:
        scen = 'Past' if y <= 2006 else 'Present'
        m = (df_dam.year == y) & (df_dam.scenario == scen)
        dam[y] = df_dam.loc[m, 'mean'].squeeze() if not df_dam.loc[m].empty else np.nan
...
def pls_vip_contrib(X, y, n_components=2):
    ...
    pls = PLSRegression(n_components=A, scale=False).fit(Xc, yc)  # avoid double-scaling
