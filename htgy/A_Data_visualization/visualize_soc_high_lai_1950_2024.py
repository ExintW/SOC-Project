...
"""
Visualize:
1) Mean LAI map (1950 to 2024) from NetCDF (CMIP6 resampled point LAI)
2) Mean SOC map (1950 to 2024 average) for ALL cells
3) Highlight cells where mean LAI > 1.231 using a red outline
4) Save annual mean SOC over high-LAI cells to CSV (optional)

Why this version:
- Your Parquet SOC files do NOT contain LAI, so LAI must come from NetCDF.
- We plot SOC everywhere, then outline the high-LAI region in red.

Outputs:
- OUTPUT_DIR/Figure/Mean_LAI_1950_2024.png
- OUTPUT_DIR/Figure/Mean_SOC_1950_2024_with_highLAI_outline.png
- OUTPUT_DIR/Annual_SOC_highLAI_1950_2024.csv
- OUTPUT_DIR/Figure/Annual_SOC_highLAI_1950_2024.png
"""
...
LAI_FILES = {
    ...
    "ssp245":     "resampled_lai_points_2015-2100_245.nc",  # used for 2015 to 2024
}
LAI_NC_DIR = PROCESSED_DIR / "CMIP6_Data_Monthly_Resampled"

# SOC monthly Parquet folders (edit if needed)
YEAR_DIRS = [
    (1950, 2006, OUTPUT_DIR / "Data" / "SOC_Past 2"),
...
OUT_SOC_MAP = FIG_DIR / "Mean_SOC_1950_2024_with_highLAI_outline.png"
...
def read_lai_points_mean_1950_2024() -> pd.DataFrame:
    ...
    Mean computed over all months within 1950 to 2024:
      historical 1950 to 2000
      present    2001 to 2014
      ssp245     2015 to 2024
    """
    parts = []
...
# =============================================================================
# (7) Compute SOC statistics (full SOC map, plus annual high-LAI mean)
# =============================================================================
...
def compute_soc_mean_map_and_annual(base_index: pd.MultiIndex, high_mask: np.ndarray):
    """
    Returns:
      - mean_soc_1d_all: 1D mean SOC over 1950 to 2024 for ALL cells
      - annual_df: DataFrame year, soc_mean_high_lai (only over high-LAI cells)
    """
    ...
    for year in range(START_YEAR, END_YEAR + 1):
        ...
        for m in range(1, 13):
            ...
            # accumulate overall mean SOC map (ALL cells)
            valid = np.isfinite(aligned_soc)
            soc_sum[valid] += aligned_soc[valid]
            soc_cnt[valid] += 1.0

            # annual mean SOC only over high-LAI cells
    ...
    mean_soc_all = np.full(len(base_index), np.nan, dtype=float)
    ok = soc_cnt > 0
    mean_soc_all[ok] = soc_sum[ok] / soc_cnt[ok]

    return mean_soc_all, pd.DataFrame(annual_rows)


# =============================================================================
# (8) Plot maps
# =============================================================================
def plot_map_imshow(grid2d: np.ndarray,
                    lon_sorted: np.ndarray,
    ...
    # colorbar axis: same height as map
...
def plot_map_imshow_with_outline(grid2d: np.ndarray,
                                 outline_mask2d: np.ndarray,
                                 lon_sorted: np.ndarray,
                                 lat_sorted_desc: np.ndarray,
                                 title: str,
                                 cbar_label: str,
                                 ...
                                 vmax=None):
    """
    Plot full grid2d, then outline outline_mask2d (True cells) using a red contour.
    """
    boundary = load_loess_border_boundary()
    ...
    # red outline for high-LAI area
    mask_float = (outline_mask2d > 0).astype(float)
    ax.contour(
        mask_float,
        levels=[0.5],
        colors="red",
...
def plot_annual_soc_line(annual_df: pd.DataFrame):
    ...
    ax.set_title(f"Annual mean SOC where mean LAI > {LAI_THRESHOLD} (1950 to 2024)")
    ax.set_xlabel("Year")
    ax.set_ylabel("Mean SOC (same unit as Total_C)")
    ax.grid(True, alpha=0.3)

    out_fp = FIG_DIR / "Annual_SOC_highLAI_1950_2024.png"
...
def main():
    ...
    # 2) Mean SOC map for ALL cells + annual series for high-LAI cells
    mean_soc_all_1d, annual_df = compute_soc_mean_map_and_annual(base_index, high_mask)
    grid_soc_all = reshape_to_grid(mean_soc_all_1d, base_index, lon_sorted, lat_sorted_desc)

    # Make 2D mask for outlining
    grid_high_mask = reshape_to_grid(high_mask.astype(float), base_index, lon_sorted, lat_sorted_desc)

    plot_map_imshow_with_outline(
        grid_soc_all,
        grid_high_mask,
        lon_sorted, lat_sorted_desc,
        title=f"Mean SOC (1950 to 2024), high LAI outlined (mean LAI > {LAI_THRESHOLD})",
        cbar_label="SOC (g/kg)",
    ...
    # Save annual CSV and a line plot
